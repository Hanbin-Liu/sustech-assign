...
ggplot(data, aes(x = species, y = value, fill = group)) + 
  ...
  scale_fill_manual(values=c("#616F69", "#A7D129"))
```
...
```{r}
myKNN <- function(data, train_col, pred_col, k, scale, M){  #scale: scale() or min-max; M: mean or median
  X <- data[, train_col]
  if(scale == "min-max"){
...
myplot <- function(n, prob){
  ...
  z <- data.frame(z)
  p <- ggplot(z) +
  geom_freqpoly(aes(z, after_stat(density), colour = 'Zn')) +
